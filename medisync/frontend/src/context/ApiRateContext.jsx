...
export function ApiRateProvider({ children }) {
  const callsRef = useRef([]) // client-side call timestamps
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const r = await fetch(
          `${BACKEND_BASE}/logs/api?window_seconds=60&rate_limit=${DEFAULT_LIMIT}`
        )
        ...
        if (alive) {
          setBackend({
            used: data.used ?? 0,
            limit: data.rate_limit ?? DEFAULT_LIMIT,
          })
        }
      } catch {
  ...
  const used = backend.used || 0

  const rate = Math.max(clientRate, used)

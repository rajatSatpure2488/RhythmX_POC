...
def test_diagnostic_report_pdf_carries_professional_fields():
    ...
    with patch.object(push_data_router, "call_configured_api", side_effect=fake_configured_call):
        result = push_data_router._upload_diagnostic_report_as_document(
...
def test_pdf_title_has_no_corrupted_dash():
    ...
    assert push_data_router._pdf_safe("Diagnostic Report — Laboratory") == "Diagnostic Report - Laboratory"
    ...
    with patch.object(push_data_router, "call_configured_api", side_effect=fake_configured_call):
        push_data_router._upload_diagnostic_report_as_document(_note23_report(), token="x", doctor_id=1, patient_id=2)
...
def test_findings_dense_paragraph_is_bulleted():
    ...
    out = push_data_router._structure_findings(text)
...
def test_findings_already_structured_is_preserved():
    echo = "Measurements:\n  EF: 72%\nImpression:\n  Concentric LVH"
    assert push_data_router._structure_findings(echo) == echo
...
def test_diagnostic_report_conclusion_code_labeled_by_vocab():
    ...
    with patch.object(push_data_router, "call_configured_api", side_effect=fake_configured_call):
        push_data_router._upload_diagnostic_report_as_document(rec, token="x", doctor_id=1, patient_id=2)

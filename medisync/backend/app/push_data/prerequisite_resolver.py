...
class PrerequisiteCache:
    ...
    def __init__(self):
        """
        Create an empty prerequisite cache.

        Parameters:
            None.

        Use case:
            Keeps doctor, office, field type, vaccine, sublab, and task category
            lookups in memory so a push run does not repeat the same EMR GET
    ...
    def is_populated(self) -> bool:
        """
        Report whether the cache has already resolved the doctor context.

        Parameters:
            None.

        Use case:
            Lets callers know whether prerequisite resolution has run at least
            once during the current backend process session.
        """
        try:
            return self._doctor is not None
        except Exception as exc:
            loguru_logger.error(f"Exception in {__name__}.is_populated: {exc}")
            raise
...
def clear_cache():
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.clear_cache: {exc}")
        raise
...
def resolve_doctor() -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.resolve_doctor: {exc}")
        raise
...
def resolve_office() -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.resolve_office: {exc}")
        raise
...
def resolve_field_types(clinical_note_template: Optional[int] = None) -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.resolve_field_types: {exc}")
        raise
...
def resolve_vaccine_inventory() -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.resolve_vaccine_inventory: {exc}")
        raise
...
def _build_vaccine_result(vaccines: list) -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._build_vaccine_result: {exc}")
        raise
...
def resolve_sublabs() -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.resolve_sublabs: {exc}")
        raise


def _build_sublab_result(sublabs: list) -> dict[str, Any]:
    """
    Build the mapper context for available sublabs.

    Parameters:
        sublabs: List of sublab records returned by the configured EMR API.

    Use case:
        Supplies a default sublab_id and display list for diagnostic report or
        lab-order payloads that require a lab partner context.
    """
    try:
        return {
        ...
        loguru_logger.error(f"Exception in {__name__}._build_sublab_result: {exc}")
        raise
...
def resolve_task_categories() -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.resolve_task_categories: {exc}")
        raise


def _build_task_cat_result(categories: list) -> dict[str, Any]:
    """
    Build the mapper context for task categories.

    Parameters:
        categories: List of task category records returned by the EMR API.

    Use case:
        Provides a default task_category_id for service-request/task payloads
        while keeping the full category list available for later matching.
    """
    try:
        return {
        ...
        loguru_logger.error(f"Exception in {__name__}._build_task_cat_result: {exc}")
        raise
...
def resolve_all() -> dict[str, Any]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.resolve_all: {exc}")
        raise

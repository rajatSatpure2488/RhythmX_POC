...
def _best_filename_alias(fname: str) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._best_filename_alias: {exc}")
        raise


def _make_log(filename: str) -> dict:
    """
    Create the default file-detection log structure for one upload.

    Parameters:
        filename: Original filename received from the upload request.

    Use case:
        Gives CSV/JSON/ZIP parsing a consistent log object that the UI can show
        for recognized and unrecognized files.
    """
    try:
        return {
            "filename":       filename,
        ...
        loguru_logger.error(f"Exception in {__name__}._make_log: {exc}")
        raise
...
def _parse_csv(content: bytes, filename: str) -> tuple[dict, dict]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._parse_csv: {exc}")
        raise
...
def _parse_json_fhir(content: bytes, filename: str = "") -> tuple[dict, dict]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._parse_json_fhir: {exc}")
        raise


def _guess_resource_type(rtype_or_name: str) -> str:
    """
    Infer the internal resource key from a FHIR resource type or filename.

    Parameters:
        rtype_or_name: FHIR resourceType value, filename, or fallback name.

    Use case:
        Converts uploaded JSON/FHIR data into the resource key used by the
        configurable mapper and push API configuration.
    """
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._guess_resource_type: {exc}")
        raise
...
def _record_identity(rec: dict) -> str:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._record_identity: {exc}")
        raise
...
def _field_count(rec: dict) -> int:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._field_count: {exc}")
        raise
...
def _merge(base: dict, extra: dict) -> dict:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._merge: {exc}")
        raise
...
def _process_file(filename: str, content: bytes) -> tuple[dict, list[dict]]:
    ...
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._process_file: {exc}")
        raise


def _extract_patient(resources: dict) -> Optional[dict]:
    """
    Extract a small patient summary from uploaded resources.

    Parameters:
        resources: Parsed resource dictionary keyed by internal resource name.

    Use case:
        Builds the patient preview shown in the UI after upload without changing
        the original parsed records used by validation and pushing.
    """
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._extract_patient: {exc}")
        raise


def _build_response(resources: dict, detection_logs: Optional[list[dict]] = None) -> dict:
    """
    Build the upload response returned to the frontend.

    Parameters:
        resources: Parsed and merged resources currently stored in session.
        detection_logs: Optional per-file detection logs from parsing.

    Use case:
        Gives the UI a complete upload summary including resource counts,
        patient preview, parsed records, and file detection details.
    """
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}._build_response: {exc}")
        raise
...
async def upload_load(files: List[UploadFile] = File(...)):
    """
    Load one or more uploaded files into the backend session.

    Parameters:
        files: CSV, JSON, or ZIP uploads sent by the frontend.

    Use case:
        Starts a new upload session by parsing files, merging duplicate records,
        and exposing resources for validation and configurable EMR push.
    """
    try:
        if not files:
        ...
        loguru_logger.error(f"Exception in {__name__}.upload_load: {exc}")
        raise
...
async def upload_load_single(file: UploadFile = File(...)):
    """
    Add one uploaded file to the existing backend session.

    Parameters:
        file: Single CSV, JSON, or ZIP upload sent by the frontend.

    Use case:
        Supports incremental uploads where each file is parsed and merged into
        the current resource session without clearing earlier files.
    """
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.upload_load_single: {exc}")
        raise
...
async def upload_clear():
    """
    Clear all uploaded resources from the backend session.

    Parameters:
        None.

    Use case:
        Lets the UI reset the upload/validation/push flow before starting with a
        different dataset or EMR configuration.
    """
    try:
        _SESSION.clear()
        return {"status": "cleared"}
    except Exception as exc:
        loguru_logger.error(f"Exception in {__name__}.upload_clear: {exc}")
        raise
...
async def upload_status():
    ...
    Use case:
        Allows the frontend to refresh counts, resource types, and file detection
        summary without re-uploading files.
    """
    try:
        ...
        loguru_logger.error(f"Exception in {__name__}.upload_status: {exc}")
        raise
